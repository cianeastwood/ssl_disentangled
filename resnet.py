"""ResNet models for PyTorch.

Borrowed from https://github.com/facebookresearch/vicreg/blob/main/resnet.py
"""
...
def adapt_resnet(model, arch_name, image_shape):
    """
    Adapts the standard torchvision resnets for different input image sizes.
    """
    image_size = image_shape[-1]
    n_channels = image_shape[0]
    if arch_name == "resnet18":
        if image_size in [32, 64, 96]:
            # Works well for 32 and 64, but unsure for 96. Perhaps 5x5 kernels for 96x96?
            print(f"Adapting kernel sizes of {arch_name} for image size {image_size}.")
            model.conv1 = nn.Conv2d(n_channels, 64, kernel_size=(3, 3), stride=(1, 1), padding=(1, 1), bias=False)
            model.maxpool = nn.Identity()
            model.avgpool = nn.AdaptiveAvgPool2d(1)
        else:
            print(f"WARNING: {arch_name} designed for 224 images being used for {image_size} images!")
    else:
        print(f"WARNING: {arch_name} designed for 224 images being used for {image_size} images!")
